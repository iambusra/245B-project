...
################# My additions
# 1. power analysis for anova (3 × 3 design; medium effect size f = 0.25)
pwr.anova.test(
  k = 3,        # 3 sentence types per context
  f = 0.25,     # medium effect size
  sig.level = 0.05,
  power = 0.80
)

# 2. t-test for matched vs contrast conditions (Cohen's d = 0.5)
pwr.t.test(
  d = 0.5,               # medium effect size
  sig.level = 0.05,
  power = 0.80,
  type = "two.sample",   # between-subjects (if collapsing mismatch and contrast)
  alternative = "two.sided"
)

# 3. linear regression: one predictor (e.g., match vs mismatch)
pwr.f2.test(
  u = 1,        # 1 predictor
  f2 = 0.15,    # small-to-medium effect
  sig.level = 0.05,
  power = 0.80
)

# 4. multiple regression: context + sentence type + interaction
pwr.f2.test(
  u = 3,        # predictors: context, sentence type, interaction
  f2 = 0.15,    # small-to-medium effect
  sig.level = 0.05,
  power = 0.80
)

# maybe??? convert f to f2 for regression power
f_value <- 0.25
f2_value <- f_value^2 / (1 - f_value^2)
f2_value

# install.packages("BayesFactor")
library(BayesFactor)

# simulate data and compute bayes factor: 
# (im a bit out of my comfort zone here, pls lmk if this is nonsensical)
set.seed(123)
n <- 40  # participants
group1 <- rnorm(n, mean = 0)
group2 <- rnorm(n, mean = 0.5)  # medium effect size (d = 0.5)

ttestBF(x = group1, y = group2)  # output: bayes Factor

# simulate bayes factors across different sample sizes
library(BayesFactor)

set.seed(123)
sample_sizes <- seq(20, 100, by = 5)
bfs <- numeric(length(sample_sizes))

for (i in seq_along(sample_sizes)) {
  n <- sample_sizes[i]
  group1 <- rnorm(n, mean = 0)
  group2 <- rnorm(n, mean = 0.5)  # Medium effect
  bf_result <- ttestBF(x = group1, y = group2)
  bfs[i] <- as.vector(bf_result@bayesFactor$bf)
}

# plot the curve
plot(sample_sizes, bfs, type = "b", pch = 19, col = "steelblue",
     ylab = "Bayes Factor", xlab = "Sample size per group",
     main = "Bayes Factor by Sample Size (d = 0.5)")
abline(h = 3, col = "pink", lty = 2)  # BF > 3 threshold


# simulate log-transformed RTs with difference
set.seed(456)
n <- 40
group1 <- rlnorm(n, meanlog = 6.2, sdlog = 0.15)
group2 <- rlnorm(n, meanlog = 6.4, sdlog = 0.15)

# log-transform before analysis
group1_log <- log(group1)
group2_log <- log(group2)

# compute bayes factor on log-transformed RTs
ttestBF(x = group1_log, y = group2_log)

# define a sequence of sample sizes per group
sample_sizes <- seq(10, 100, by = 5)

# compute power for each sample size
powers <- sapply(sample_sizes, function(n) {
  pwr.anova.test(k = 3, n = n, f = 0.25, sig.level = 0.05)$power
})

# plot the power curve
plot(sample_sizes, powers, type = "b", pch = 19, col = "steelblue",
     xlab = "Sample size per group", ylab = "Power",
     main = "Power Curve for One-Way ANOVA (f = 0.25)")
abline(h = 0.80, col = "pink", lty = 2)  # 80% power threshold
